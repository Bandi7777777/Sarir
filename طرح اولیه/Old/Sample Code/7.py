...
class ProfessionalUI(QMainWindow):
    ...
    def toggle_sidebar(self, event):
        width = self.centralWidget().layout().itemAt(0).widget().width()
        if width == 250:
            self.sidebar_anim.setStartValue(QRect(0, 0, 250, self.height()))
            self.sidebar_anim.setEndValue(QRect(-250, 0, 250, self.height()))
        else:
            self.sidebar_anim.setStartValue(QRect(-250, 0, 250, self.height()))
            self.sidebar_anim.setEndValue(QRect(0, 0, 250, self.height()))
