...
def main():
    ...
    merged_dir = base / "outputs" / "merged_parts"
    if not merged_dir.exists():
        raise SystemExit("merged_parts not found. Run exporter first.")
    ...
    single_md = out_root / f"SARIR_Export_{ts}.md"
    zip_path  = out_root / f"SARIR_Export_{ts}.zip"
    with single_md.open("w", encoding="utf-8") as out:
        out.write(f"# SARIR Consolidated Export\n- Generated: {datetime.now().isoformat(timespec='seconds')}\n\n---\n")
        for p in parts:
            out.write(f"\n\n<!-- {p.name} -->\n\n")
