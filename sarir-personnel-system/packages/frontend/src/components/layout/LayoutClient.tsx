...
// فقط خود سایدبار، برای اینکه از useSidebar استفاده کنیم
function NeonSidebar({ pathname }: { pathname: string }) {
    ...
    >
      <div
        className="
          ...
          bg-gradient-to-b from-[#020617] via-[#050922] to-[#020617]
          text-slate-100 shadow-[0_0_40px_rgba(0,0,0,0.9)]
        "
      >
        {/* لوگو بالای سایدبار */}
                ...
                <SidebarMenuItem key={item.href}>
                  ...
                  >
                    <Link href={item.href} className="flex items-center gap-2">
                      ...
                      {/* وقتی سایدبار collapse شده، این span خودش توسط lib مخفی می‌شه */}
      ...
      {/* ریل برای باز/بسته کردن در دسکتاپ */}
      <SidebarRail />
    </Sidebar>
  );
}
...
export default function LayoutClient({ children }: LayoutClientProps) {
  ...
  // صفحات لاگین / احراز هویت بدون سایدبار
  const isAuthRoute =
  ...
  return (
    <SidebarProvider
      defaultOpen={false} // شروع به صورت آیکن‌محور
      className="flex min-h-screen w-screen overflow-hidden bg-[#020617]"
      dir="rtl"
    >
      {/* محتوای اصلی؛ همیشه تمام عرض باقی‌مانده و تمام ارتفاع */}
      <main className="flex-1 min-h-screen bg-[#020617]" dir="rtl">
        {children}
      </main>

      {/* سایدبار سمت راست */}
      <NeonSidebar pathname={pathname} />
    </SidebarProvider>
  );
}
